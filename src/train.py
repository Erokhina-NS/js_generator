...
class ModelTrainer:
  ...
  def init_model(self):
    ...
    print_msg('vocab_size='+str(vocab_size), 'MY', self._help_dir+'help_train', True)
  ...
  def print_config(self):
    ...
    print_msg(msg, 'INFO',self._help_dir+'help_train', True)
  ...
  def split_data(self, seed_data):
    ...
    test = seed_data[:size]
    print_msg('Train data COUNT='+str(len(train)), 'INFO', self._help_dir + 'help_preproc', True)
    print_msg('Test data COUNT='+str(len(test)), 'INFO', self._help_dir + 'help_preproc', True)
...
def main():  
  ...
  config_path = MAIN_PATH+'js_generator/conf-v8.json'
  conf = Config(config_path, MAIN_PATH)
  print(f"Start settings:\n Engine: {conf.eng_name}\n Data dir: {conf.data_dir}\n###############################################################################\n")
  
  cuda=torch.device('cuda')
  trainer = ModelTrainer(conf)
  with open(trainer._help_dir+'/help_train', 'w') as f:
    f.write('')
  print(f"Start settings:\n Train data path: {trainer._train_data_path}\n Model dir: {trainer._model_dir}\n Help dir: {trainer._help_dir}\n###############################################################################\n")
