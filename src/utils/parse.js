...
function parse(src, des, file, count){
  ast_path = extract_name(file);
  ast_path = path.join(des, ast_path);
  // fs.appendFileSync('/home/jovyan/js_generator/data_jerry/help/help_preproc', count+'='+ast_path+'\n');
  js_path = path.join(src, file);
  code = read_file(js_path);

  try {
    parse_and_write(code, ast_path);
    // fs.appendFileSync('/home/jovyan/js_generator/data_jerry/help/help_preproc', 'OK\n');
  } catch (err) {
    err_msg = '[!] Error - ' + js_path + '\n';
    err_msg += err
    // fs.appendFileSync('/home/jovyan/js_generator/data_jerry/help/help_preproc', err_msg+'\n');
    console.log(err_msg);
  }
}
