...
def build_id_map(conf):
  print_msg('[1/2] Building def dictionary','INFO',conf.help_dir+'help_train')
  ast_dir = parse_seed(conf)
  def_dict = build_dict(ast_dir)

  print_msg('[2/2] Building ID map','INFO',conf.help_dir+'help_train')
...
def construct_map(conf, def_dict):
  ...
  for idx, file_name in enumerate(harness_keys):
    ...
    print_msg(msg, 'INFO',conf.help_dir+'help_train')
...
def parse_seed(conf):
  ...
  make_dir(ast_dir)
  parser.parse(seed_dir, ast_dir, conf.help_dir)
  return ast_dir
